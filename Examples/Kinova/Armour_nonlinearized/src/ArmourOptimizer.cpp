        ...
        if (num_fixed_joints > 0) {
            try {
                ...
                for (i = 0; i < num_time_steps; i++) {
                    ...
                    Interval f_c_x = dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].linear()(0).slice(x);
                    Number f_c_x_center = getCenter(f_c_x);
                    Number f_c_x_radius = getRadius(f_c_x);

                    Interval f_c_y = dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].linear()(1).slice(x);
                    Number f_c_y_center = getCenter(f_c_y);
                    Number f_c_y_radius = getRadius(f_c_y);

                    Interval f_c_z = dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].linear()(2).slice(x);
                    Number f_c_z_center = getCenter(f_c_z);
                    Number f_c_z_radius = getRadius(f_c_z);

                    Interval n_c_x = dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].angular()(0).slice(x);
                    Number n_c_x_center = getCenter(n_c_x);
                    Number n_c_x_radius = getRadius(n_c_x);

                    Interval n_c_y = dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].angular()(1).slice(x);
                    Number n_c_y_center = getCenter(n_c_y);
                    Number n_c_y_radius = getRadius(n_c_y);

                    Interval n_c_z = dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].angular()(2).slice(x);
                    Number n_c_z_center = getCenter(n_c_z);
                    Number n_c_z_radius = getRadius(n_c_z);

                    Eigen::Vector3<PZSparse> ZMP_top;
                    ZMP_top(0) = dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].angular()(1) * 1.0;
                    ZMP_top(1) = -dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].angular()(0) * 1.0;
                    ZMP_top(2) = dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].angular()(0) * 0.0;

                    Interval ZMP_x = ZMP_top(0).slice(x);

                    Number ZMP_top_x_center = getCenter(ZMP_x);
                    Number ZMP_top_x_radius = getRadius(ZMP_x);

                    Interval ZMP_y = ZMP_top(1).slice(x);

                    Number ZMP_top_y_center = getCenter(ZMP_y);
                    Number ZMP_top_y_radius = getRadius(ZMP_y);

                    Interval ZMP_z = ZMP_top(2).slice(x);

                    Number ZMP_top_z_center = getCenter(ZMP_z);
                    Number ZMP_top_z_radius = getRadius(ZMP_z);

                    Interval ZMP_bottom = dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].linear()(2).slice(x);
                    Number ZMP_bottom_center = getCenter(ZMP_bottom);
                    Number ZMP_bottom_radius = getRadius(ZMP_bottom);

                    Number u_s = robotInfoPtr_->mu;
                    Number surf_rad = robotInfoPtr_->contact_surface_radius;

                    // slipping constraint
                    if ( (f_c_x_center >= 0) && (f_c_y_center >= 0) && (f_c_z_center >= 0) ){
                        // Note: double check that the center/radius is a number that can be squared
                        g[i * NUM_CONTACT_CONSTRAINTS + 1 + offset] = pow(f_c_x_center,2) - 2*f_c_x_radius*f_c_x_center - pow(f_c_x_radius,2) + pow(f_c_y_center,2) - 2*f_c_y_radius*f_c_y_center - pow(f_c_y_radius,2) - pow(u_s,2) * ( pow(f_c_z_center,2) + 2*f_c_z_radius*f_c_z_center + pow(f_c_z_radius,2)); // checked signs
                ...
                // Note: double check that the center/radius is a number that can be squared
                        g[i * NUM_CONTACT_CONSTRAINTS + 1 + FRICTION_CONE_LINEARIZED_SIZE + offset] = pow(ZMP_top_x_center,2) - 2*ZMP_top_x_radius*ZMP_top_x_center - pow(ZMP_top_x_radius,2) + pow(ZMP_top_y_center,2) - 2*ZMP_top_y_radius*ZMP_top_y_center - pow(ZMP_top_y_radius,2) - pow(surf_rad,2) * ( pow(ZMP_bottom_center,2) + 2*ZMP_bottom_radius*ZMP_bottom_center + pow(ZMP_bottom_radius,2)); // checked signs
                    ...
                    g[i * NUM_CONTACT_CONSTRAINTS + 1 + offset] = -g[i * NUM_CONTACT_CONSTRAINTS + 1 + offset];
                    g[i * NUM_CONTACT_CONSTRAINTS + 1 + FRICTION_CONE_LINEARIZED_SIZE + offset] = -g[i * NUM_CONTACT_CONSTRAINTS + 1 + FRICTION_CONE_LINEARIZED_SIZE + offset];
                }
            }
            catch (const std::exception& e) {
                std::cerr << e.what() << std::endl;
        ...
        if (num_fixed_joints > 0) {
            try {
                ...
                for (i = 0; i < num_time_steps; i++) {
                    ...
                    dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].linear()(0).slice(f_c_x_grad, x);
                    dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].linear()(1).slice(f_c_y_grad, x);
                    dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].linear()(2).slice(f_c_z_grad, x);
                    ZMP_top(0).slice(ZMP_top_x_grad, x);
                    ZMP_top(1).slice(ZMP_top_y_grad, x);
                    dynPtr_->data_sparses[i].f[dynPtr_->model_sparses[i].nv].linear()(2).slice(ZMP_bottom_grad, x);
                    ...
                    if ( (f_c_x_center >= 0) && (f_c_y_center >= 0) && (f_c_z_center >= 0) ){
                        for (int j=0;j<NUM_FACTORS;j++) {
                            values[(i * NUM_CONTACT_CONSTRAINTS + 1) * NUM_FACTORS + offset + j] = 2*f_c_x_center*f_c_x_grad[j] + 2*f_c_x_radius*f_c_x_grad[j] + 2*f_c_y_center*f_c_y_grad[j] + 2*f_c_y_radius*f_c_y_grad[j] - pow(u_s,2) * ( 2*f_c_z_center*f_c_z_grad[j] - 2*f_c_z_radius*f_c_z_grad[j] );
                        }
                    }
                    // condition 2: y negative
                    else if ( (f_c_x_center >= 0) && (f_c_y_center <= 0) && (f_c_z_center >= 0) ) {
                    ...
                    // condition 8: x and y and z negative
                        ...
                        for (int j=0;j<NUM_FACTORS;j++) {
                            values[(i * NUM_CONTACT_CONSTRAINTS + 1) * NUM_FACTORS + offset + j] = 2*f_c_x_center*f_c_x_grad[j] - 2*f_c_x_radius*f_c_x_grad[j] + 2*f_c_y_center*f_c_y_grad[j] - 2*f_c_y_radius*f_c_y_grad[j] - u_s*u_s * ( 2*f_c_z_center*f_c_z_grad[j] + 2*f_c_z_radius*f_c_z_grad[j] );
                        }
                    for (int j=0;j<NUM_FACTORS;j++) {
                            values[(i * NUM_CONTACT_CONSTRAINTS + 1) * NUM_FACTORS + offset + j] = -values[(i * NUM_CONTACT_CONSTRAINTS + 1) * NUM_FACTORS + offset + j];
                    }
                    }
                    ...
                    if ( (ZMP_top_x_center >= 0) && (ZMP_top_y_center >= 0) && (ZMP_bottom_center >= 0) ){
                        for (int j=0;j<NUM_FACTORS;j++) {
                            values[(i * NUM_CONTACT_CONSTRAINTS + 1 + FRICTION_CONE_LINEARIZED_SIZE) * NUM_FACTORS + offset + j] = 2*ZMP_top_x_center*ZMP_top_x_grad[j] + 2*ZMP_top_x_radius*ZMP_top_x_grad[j] + 2*ZMP_top_y_center*ZMP_top_y_grad[j] + 2*ZMP_top_y_radius*ZMP_top_y_grad[j] - pow(surf_rad,2) * ( 2*ZMP_bottom_center*ZMP_bottom_grad[j] - 2*ZMP_bottom_radius*ZMP_bottom_grad[j]);
                        }
                    }
                    // condition 2: y negative
                    else if ( (ZMP_top_x_center >= 0) && (ZMP_top_y_center <= 0) && (ZMP_bottom_center >= 0) ) {
                    ...
                    for (int j=0;j<NUM_FACTORS;j++) {
                            values[(i * NUM_CONTACT_CONSTRAINTS + 1 + FRICTION_CONE_LINEARIZED_SIZE) * NUM_FACTORS + offset + j] = -values[(i * NUM_CONTACT_CONSTRAINTS + 1 + FRICTION_CONE_LINEARIZED_SIZE) * NUM_FACTORS + offset + j];
                    }
                }
            }
            catch (const std::exception& e) {
