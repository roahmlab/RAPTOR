...
int main(int argc, char* argv[]) {
    ...
    // load the training pos, vel, acc, tau data
    ...
    try {
        ...
        // Compare the solution with urdf
        ...
        std::cout << "Solution: " << (mynlp->solution.head(70)).transpose() << std::endl;
        std::cout << "Urdf parameters" << qrSolverPtr_->phi.transpose() << std::endl;
        std::cout << "The different between urdf parameters and solution"  << std::endl << "joint 1: ";
        for (int i = 0; i < phi_diff.size(); ++i) {
            std::cout << phi_diff(i) << " ";
            if ((i + 1) % 10 == 0) {
                if ((i+1)/10 +1 <= model.nv ){
                    std::cout << std::endl << "joint " << (i+1)/10 +1<< ": ";
                }
            }
        }

        // write the full parameters into the file 
        ...
        // write the estimate torque into the file 
