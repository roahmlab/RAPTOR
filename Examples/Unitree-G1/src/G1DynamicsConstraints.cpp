...
G1DynamicsConstraints::G1DynamicsConstraints(const std::shared_ptr<Model>& modelPtr_input, 
...
void G1DynamicsConstraints::reinitialize() {
    // swap the stance leg
    if (stanceLeg == 'L' || stanceLeg == 'l') {
        stanceLeg = 'R';
    }
...
int G1DynamicsConstraints::return_dependent_joint_index(const int id) {
    assert(0 <= id && id < NUM_DEPENDENT_JOINTS);
    return dependentJointIds[id];
}

int G1DynamicsConstraints::return_independent_joint_index(const int id) {
    assert(0 <= id && id < NUM_INDEPENDENT_JOINTS);
    return independentJointIds[id];
}

void G1DynamicsConstraints::fill_dependent_vector(VecX& r, const VecX& v, const bool setZero) {
...
void G1DynamicsConstraints::fill_independent_vector(VecX& r, const VecX& v, const bool setZero) {
    assert(r.size() == modelPtr_->nv);
...
void G1DynamicsConstraints::fill_dependent_columns(MatX& r, const MatX& m, const bool setZero) {
...
void G1DynamicsConstraints::fill_independent_columns(MatX& r, const MatX& m, const bool setZero) {
...
void G1DynamicsConstraints::fill_dependent_rows(MatX& r, const MatX& m, const bool setZero) {
...
void G1DynamicsConstraints::fill_independent_rows(MatX& r, const MatX& m, const bool setZero) {
...
Eigen::VectorXd G1DynamicsConstraints::get_dependent_vector(const VecX& v) {
...
Eigen::VectorXd G1DynamicsConstraints::get_independent_vector(const VecX& v) {
    assert(v.size() == modelPtr_->nv);
...
void G1DynamicsConstraints::get_dependent_columns(MatX& r, const MatX& m) {
...
void G1DynamicsConstraints::get_independent_columns(MatX& r, const MatX& m) {
    assert(m.cols() == modelPtr_->nv);
...
void G1DynamicsConstraints::get_dependent_rows(MatX& r, const MatX& m) {
...
void G1DynamicsConstraints::get_independent_rows(MatX& r, const MatX& m) {
    assert(m.rows() == modelPtr_->nv);
...
void G1DynamicsConstraints::setupJointPosition(VecX& q, bool compute_derivatives) {
...
void G1DynamicsConstraints::get_c(const VecX& q) {
    fkPtr_->compute(0, 
                    contact_joint_id, 
                    q, 
                    nullptr, 
...
void G1DynamicsConstraints::get_J(const VecX& q) {
...
void G1DynamicsConstraints::get_Jx_partial_dq(const VecX& q, const VecX& x) {
...
void G1DynamicsConstraints::get_JTx_partial_dq(const VecX& q, const VecX& x) {
...
void G1DynamicsConstraints::get_Jxy_partial_dq(const VecX& q, const VecX& x, const VecX& y) {
    assert(x.size() == modelPtr_->nv);
