...
// // constructor
// G1MultipleStepOptimizer::G1MultipleStepOptimizer()
// {
// }
...
bool G1MultipleStepOptimizer::set_parameters(
...
) 
{
    ...
    for (int i = 0; i < NSteps_input; i++) {
        stepOptVec_.push_back(new G1SingleStepOptimizer());
    ...
    for (int i = 0; i < NSteps_input - 1; i++) {
        periodConsVec_.push_back(
            std::make_shared<G1MultipleStepPeriodicityConstraints>(
...
// [TNLP_get_nlp_info]
bool G1MultipleStepOptimizer::get_nlp_info(
   Index&          n,
   Index&          m,
   Index&          nnz_jac_g,
   Index&          nnz_h_lag,
...
bool G1MultipleStepOptimizer::get_bounds_info(
    Index   n,
    Number* x_l,
    Number* x_u,
    Index   m,
...
bool G1MultipleStepOptimizer::eval_f(
   Index         n,
   const Number* x,
   bool          new_x,
   Number&       obj_value
...
bool G1MultipleStepOptimizer::eval_grad_f(
   Index         n,
   const Number* x,
   bool          new_x,
   Number*       grad_f
...
bool G1MultipleStepOptimizer::eval_g(
    Index         n,
    const Number* x,
    bool          new_x,
    Index         m,
...
bool G1MultipleStepOptimizer::eval_jac_g(
   Index         n,
   const Number* x,
   bool          new_x,
   Index         m,
...
void G1MultipleStepOptimizer::summarize_constraints(
    Index                      m,
    const Number*              g,
    const bool                 verbose
) 
