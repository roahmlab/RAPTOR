...
void DigitWholeBodyDynamicsConstraints::reinitialize(const char stanceLeg_input) {
...
void DigitWholeBodyDynamicsConstraints::setupJointPosition(VecX& q, bool compute_derivatives) {
    ...
    const size_t n = NUM_DEPENDENT_JOINTS - 6;
    ...
    for (int i = 0; i < NUM_DEPENDENT_JOINTS - 6; i++) {
        gsl_vector_set(x, i, qcopy(dependentJointIds[i + 6]));
    }
    ...
    for (int i = 0; i < NUM_DEPENDENT_JOINTS - 6; i++) {
        qcopy(dependentJointIds[i + 6]) = gsl_vector_get(s->x, i);
    }
...
int fillDependent_f(const gsl_vector* x, void *params, gsl_vector* f) {
    DigitWholeBodyDynamicsConstraints* constraintsData = (DigitWholeBodyDynamicsConstraints*)params;

    for (int i = 0; i < NUM_DEPENDENT_JOINTS - 6; i++) {
        constraintsData->qcopy(constraintsData->dependentJointIds[i + 6]) = gsl_vector_get(x, i);
    }
...
int fillDependent_df(const gsl_vector* x, void *params, gsl_matrix* J) {
    ...
    for (int i = 0; i < NUM_DEPENDENT_JOINTS - 6; i++) {
        for (int j = 0; j < NUM_DEPENDENT_JOINTS - 6; j++) {
            gsl_matrix_set(J, i, j, constraintsData->J(i, constraintsData->dependentJointIds[j + 6]));
        }
    }
...
void DigitWholeBodyDynamicsConstraints::get_c(const VecX& q) {
    double t2 = cos(q(9));
    double t3 = cos(q(10));
    double t4 = cos(q(11));
    double t5 = cos(q(12));
...
void DigitWholeBodyDynamicsConstraints::get_Jx_partial_dq(const VecX& q, const VecX& x) {
...
void DigitWholeBodyDynamicsConstraints::get_JTx_partial_dq(const VecX& q, const VecX& x) {
...
void DigitWholeBodyDynamicsConstraints::get_Jxy_partial_dq(const VecX& q, const VecX& x, const VecX& y) {
    assert(x.size() == modelPtr_->nv);
