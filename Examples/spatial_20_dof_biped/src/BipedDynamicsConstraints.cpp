...
BipedDynamicsConstraints::BipedDynamicsConstraints(const std::shared_ptr<Model>& modelPtr_input, 
...
void BipedDynamicsConstraints::reinitialize() {
    // swap the stance leg
    if (stanceLeg == 'L' || stanceLeg == 'l') {
        stanceLeg = 'R';
    }
...
int BipedDynamicsConstraints::return_dependent_joint_index(const int id) {
    assert(0 <= id && id < NUM_DEPENDENT_JOINTS);
    return dependentJointIds[id];
}

int BipedDynamicsConstraints::return_independent_joint_index(const int id) {
    assert(0 <= id && id < NUM_INDEPENDENT_JOINTS);
    return independentJointIds[id];
}

void BipedDynamicsConstraints::fill_dependent_vector(VecX& r, const VecX& v, const bool setZero) {
...
void BipedDynamicsConstraints::fill_independent_vector(VecX& r, const VecX& v, const bool setZero) {
    assert(r.size() == modelPtr_->nv);
...
void BipedDynamicsConstraints::fill_dependent_columns(MatX& r, const MatX& m, const bool setZero) {
...
void BipedDynamicsConstraints::fill_independent_columns(MatX& r, const MatX& m, const bool setZero) {
...
void BipedDynamicsConstraints::fill_dependent_rows(MatX& r, const MatX& m, const bool setZero) {
...
void BipedDynamicsConstraints::fill_independent_rows(MatX& r, const MatX& m, const bool setZero) {
...
Eigen::VectorXd BipedDynamicsConstraints::get_dependent_vector(const VecX& v) {
...
Eigen::VectorXd BipedDynamicsConstraints::get_independent_vector(const VecX& v) {
    assert(v.size() == modelPtr_->nv);
...
void BipedDynamicsConstraints::get_dependent_columns(MatX& r, const MatX& m) {
...
void BipedDynamicsConstraints::get_independent_columns(MatX& r, const MatX& m) {
    assert(m.cols() == modelPtr_->nv);
...
void BipedDynamicsConstraints::get_dependent_rows(MatX& r, const MatX& m) {
...
void BipedDynamicsConstraints::get_independent_rows(MatX& r, const MatX& m) {
    assert(m.rows() == modelPtr_->nv);
...
void BipedDynamicsConstraints::setupJointPosition(VecX& q, bool compute_derivatives) {
...
void BipedDynamicsConstraints::get_c(const VecX& q) {
    fkPtr_->compute(0, 
                    contact_joint_id, 
                    q, 
                    nullptr, 
...
void BipedDynamicsConstraints::get_J(const VecX& q) {
...
void BipedDynamicsConstraints::get_Jx_partial_dq(const VecX& q, const VecX& x) {
...
void BipedDynamicsConstraints::get_JTx_partial_dq(const VecX& q, const VecX& x) {
...
void BipedDynamicsConstraints::get_Jxy_partial_dq(const VecX& q, const VecX& x, const VecX& y) {
    assert(x.size() == modelPtr_->nv);
