...
// // constructor
// BipedSingleStepOptimizer::BipedSingleStepOptimizer()
// {
// }
...
bool BipedSingleStepOptimizer::set_parameters(
    const VecX& x0_input,
 ...
 ) 
{
    ...
    dcidPtr_ = std::make_shared<BipedConstrainedInverseDynamics>(model_input, 
                                                                 trajPtr_,
    ...
    constraintsPtrVec_.push_back(std::make_unique<BipedCustomizedConstraints>(model_input, 
                                                                              trajPtr_, 
                                                                              dcidPtr_->ddcPtr_,
                                                                              gp_input));    
    ...
    if (periodic) {
        constraintsPtrVec_.push_back(std::make_unique<BipedSingleStepPeriodicityConstraints>(trajPtr_, 
                                                                                             dcidPtr_,
                                                                                             FRICTION_PARAMS));    
...
// [TNLP_get_nlp_info]
bool BipedSingleStepOptimizer::get_nlp_info(
   Index&          n,
   Index&          m,
   Index&          nnz_jac_g,
   Index&          nnz_h_lag,
