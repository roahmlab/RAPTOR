...
BipedCustomizedConstraints::BipedCustomizedConstraints(const Model& model_input,
                                                       std::shared_ptr<Trajectories>& trajPtr_input,
                                                       std::shared_ptr<BipedDynamicsConstraints>& ddcPtr_input,
...
void BipedCustomizedConstraints::compute(const VecX& z, 
                                         bool compute_derivatives,
                                         bool compute_hessian) {
    if (compute_hessian) {
        throw std::invalid_argument("BipedCustomizedConstraints does not support hessian computation");
    }
    ...
    // (4) torso height always larger than 0.3 meter
...
void BipedCustomizedConstraints::compute_bounds() {
    // (1) swingfoot height always larger than 0
    //               height equal to 0 at the beginning and at the end
    //               height higher than the desired value in the middle
    // g1_lb = VecX::Zero(trajPtr_->N);
    g1_lb = VecX::Constant(trajPtr_->N, -1e19);
    ...
    g2_lb = VecX::Constant(trajPtr_->N, -1e19);
    g2_ub = VecX::Constant(trajPtr_->N, 1e19);
    g3_lb = VecX::Constant(trajPtr_->N, -1e19);
    g3_ub = VecX::Constant(trajPtr_->N, 1e19);
    g4_lb = VecX::Constant(trajPtr_->N, -1e19);
    g4_ub = VecX::Constant(trajPtr_->N, 1e19);
    ...
    //     roll and pitch always close to 0
    //     yaw always close to 0 when walking forward
    g6_lb = VecX::Constant(trajPtr_->N, 0.2);
    ...
    g7_lb = VecX::Constant(trajPtr_->N, -1e19);
    g7_ub = VecX::Constant(trajPtr_->N, 1e19);
    g8_lb = VecX::Constant(trajPtr_->N, -1e19);
    g8_ub = VecX::Constant(trajPtr_->N, 1e19);
    g9_lb = VecX::Constant(trajPtr_->N, -1e19);
    g9_ub = VecX::Constant(trajPtr_->N, 1e19);

    g_lb << g1_lb, g2_lb, g3_lb, g4_lb, g5_lb, g6_lb, g7_lb, g8_lb, g9_lb;
    g_ub << g1_ub, g2_ub, g3_ub, g4_ub, g5_ub, g6_ub, g7_ub, g8_ub, g9_ub;
}

void BipedCustomizedConstraints::print_violation_info() {
    // (1) swingfoot height always larger than 0
    //               height equal to 0 at the beginning and at the end
    //               height higher than the desired value in the middle
    for (int i = 0; i < trajPtr_->N; i++) {
        if (g1(i) <= g1_lb(i) - 1e-4) {
            std::cout << "        BipedCustomizedConstraints.cpp: swing foot height at time instance " 
                      << i 
    ...
    // for (int i = 0; i < trajPtr_->N; i++) {
    //     if (abs(g2(i)) >= 1e-4) {
    //         std::cout << "        BipedCustomizedConstraints.cpp: swing foot roll at time instance " 
    //                   << i 
    //                   << " is not close to 0: " 
    //                   << g2(i) 
    //                   << std::endl;
    //     }
    //     if (abs(g3(i)) >= 1e-4) {
    //         std::cout << "        BipedCustomizedConstraints.cpp: swing foot pitch at time instance " 
    //                   << i 
    //                   << " is not close to 0: " 
    //                   << g3(i) 
    //                   << std::endl;
    //     }
    //     if (abs(g4(i)) >= 1e-4) {
    //         std::cout << "        BipedCustomizedConstraints.cpp: swing foot yaw at time instance " 
    //                   << i 
    //                   << " is not close to 0: " 
    //                   << g4(i) 
    //                   << std::endl;
    ...
    if (g5(0) <= g5_lb(0) - 1e-4 ||
        g5(0) >= g5_ub(0) + 1e-4) {
        std::cout << "        BipedCustomizedConstraints.cpp: swing foot x at beginning is not equal to desired value: " 
                  << g5(0) 
                  << std::endl;
    }
    if (g5(1) <= g5_lb(1) - 1e-4 ||
        g5(1) >= g5_ub(1) + 1e-4) {
        std::cout << "        BipedCustomizedConstraints.cpp: swing foot x at end is not equal to desired value: " 
                  << g5(1) 
                  << std::endl;
    }
    ...
    for (int i = 0; i < trajPtr_->N; i++) {
        if (g6(i) <= g6_lb(i) - 1e-4) {
            std::cout << "        BipedCustomizedConstraints.cpp: torso height at time instance " 
                      << i 
                      << " is below lower limit: " 
                      << g6(i) 
                      << " < " 
        ...
        // if (abs(g7(i)) >= 1e-4) {
        //     std::cout << "        BipedCustomizedConstraints.cpp: torso roll at time instance " 
        //               << i 
        //               << " is not close to 0: " 
        //               << g7(i) 
        //               << std::endl;
        // }
        // if (abs(g8(i)) >= 1e-4) {
        //     std::cout << "        BipedCustomizedConstraints.cpp: torso pitch at time instance " 
        //               << i 
        //               << " is not close to 0: " 
        //               << g8(i) 
        //               << std::endl;
        // }
        // if (abs(g9(i)) >= 1e-4) {
        //     std::cout << "        BipedCustomizedConstraints.cpp: torso yaw at time instance " 
        //               << i 
        //               << " is not close to 0: " 
        //               << g9(i) 
        //               << std::endl;
