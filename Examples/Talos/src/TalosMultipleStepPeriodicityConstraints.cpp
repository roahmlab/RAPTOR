...
TalosMultipleStepPeriodicityConstraints::TalosMultipleStepPeriodicityConstraints(std::shared_ptr<Trajectories>& currTrajPtr_input,
    ...
    if (currTrajPtr_->varLength != nextTrajPtr_->varLength) {
        throw std::invalid_argument("TalosMultipleStepPeriodicityConstraints: currTrajPtr_ and nextTrajPtr_ should have the same varLength");
    }
...
void TalosMultipleStepPeriodicityConstraints::compute(const VecX& z, 
                                                      bool compute_derivatives,
                                                      bool compute_hessian) {
    if (compute_hessian) {
        throw std::invalid_argument("TalosMultipleStepPeriodicityConstraints does not support hessian computation");
    }

    Model& model = *(currDcidPtr_->modelPtr_);
    Data& data = *(currDcidPtr_->dataPtr_);
    std::shared_ptr<TalosDynamicsConstraints> nextDdcPtr_ = nextDcidPtr_->ddcPtr_;
...
void TalosMultipleStepPeriodicityConstraints::compute_bounds() {
...
void TalosMultipleStepPeriodicityConstraints::print_violation_info() {
    // (1) H * (v+ - v-) = J * lambda
    for (int i = 0; i < NUM_JOINTS; i++) {
        if (abs(g1(i)) >= 1e-4) {
            std::cout << "        TalosMultipleStepPeriodicityConstraints.cpp: H * (v+ - v-) = J * lambda: dim " 
                      << i 
                      << " is violated: "
                      << g(i) 
                      << std::endl;
    ...
    for (int i = 0; i < NUM_DEPENDENT_JOINTS; i++) {
        if (abs(g2(i)) >= 1e-4) {
            std::cout << "        TalosMultipleStepPeriodicityConstraints.cpp: J * v+ = 0: dim " 
                      << i
                      << " is violated: "
                      << g2(i)
                      << std::endl;
    ...
    for (int i = 0; i < NUM_INDEPENDENT_JOINTS; i++) {
        if (abs(g3(i)) >= 1e-4) {
            std::cout << "        TalosMultipleStepPeriodicityConstraints.cpp: position reset: dim " 
                      << i
                      << " is violated: "
                      << g3(i)
                      << std::endl;
    ...
    for (int i = 0; i < NUM_INDEPENDENT_JOINTS; i++) {
        if (abs(g4(i)) >= 1e-4) {
            std::cout << "        TalosMultipleStepPeriodicityConstraints.cpp: velocity reset: dim " 
                      << i
                      << " is violated: "
                      << g4(i)
                      << std::endl;
    ...
    // (5) contact constraints
    if (g5(0) <= -1e-4) {
        std::cout << "        TalosMultipleStepPeriodicityConstraints.cpp: positive contact force is violated: " 
                  << g5(0)
                  << std::endl;
    }
    if (g5(1) >= 1e-4) {
        std::cout << "        TalosMultipleStepPeriodicityConstraints.cpp: translation friction cone is violated: " 
                  << g5(1)
                  << std::endl;
    }
    if (g5(2) >= 1e-4) {
        std::cout << "        TalosMultipleStepPeriodicityConstraints.cpp: rotation friction cone is violated: " 
                  << g5(2)
                  << std::endl;
    }
    if (g5(3) >= 1e-4) {
        std::cout << "        TalosMultipleStepPeriodicityConstraints.cpp: ZMP on one axis is violated: " 
                  << g5(3)
                  << std::endl;
    }
    if (g5(4) >= 1e-4) {
    ...
    if (g5(5) >= 1e-4) {
        std::cout << "        TalosMultipleStepPeriodicityConstraints.cpp: ZMP on the other axis is violated: " 
                  << g5(5)
                  << std::endl;
    }
    if (g5(6) >= 1e-4) {
