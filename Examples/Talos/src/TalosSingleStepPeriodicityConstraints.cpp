...
TalosSingleStepPeriodicityConstraints::TalosSingleStepPeriodicityConstraints(std::shared_ptr<Trajectories>& trajPtr_input,
...
void TalosSingleStepPeriodicityConstraints::compute(const VecX& z, 
                                                    bool compute_derivatives,
                                                    bool compute_hessian) {
    if (compute_hessian) {
        throw std::invalid_argument("TalosSingleStepPeriodicityConstraints does not support hessian computation");
    }
    ...
    if (compute_derivatives) {
        ...
        const float original_gravity = dcidPtr_->modelPtr_->gravity.linear()(2);
...
void TalosSingleStepPeriodicityConstraints::compute_bounds() {
...
void TalosSingleStepPeriodicityConstraints::print_violation_info() {
    ...
    for (int i = 0; i < NUM_JOINTS; i++) {
        if (abs(g1(i)) > 1e-5) {
            std::cout << "        TalosSingleStepPeriodicityConstraints.cpp: H * (v+ - v-) = J * lambda: dim " 
                      << i 
                      << " is violated: "
                      << g(i) 
                      << std::endl;
    ...
    for (int i = 0; i < NUM_DEPENDENT_JOINTS; i++) {
        if (abs(g2(i)) > 1e-5) {
            std::cout << "        TalosSingleStepPeriodicityConstraints.cpp: J * v+ = 0: dim " 
                      << i
                      << " is violated: "
                      << g2(i)
                      << std::endl;
    ...
    for (int i = 0; i < NUM_INDEPENDENT_JOINTS; i++) {
        if (abs(g3(i)) > 1e-5) {
            std::cout << "        TalosSingleStepPeriodicityConstraints.cpp: position reset: dim " 
                      << i
                      << " is violated: "
                      << g3(i)
                      << std::endl;
    ...
    for (int i = 0; i < NUM_INDEPENDENT_JOINTS; i++) {
        if (abs(g4(i)) > 1e-5) {
            std::cout << "        TalosSingleStepPeriodicityConstraints.cpp: velocity reset: dim " 
                      << i
                      << " is violated: "
                      << g4(i)
                      << std::endl;
    ...
    // (5) contact constraints
    if (g5(0) <= -1e-4) {
        std::cout << "        TalosSingleStepPeriodicityConstraints.cpp: positive contact force is violated: " 
                  << g5(0)
                  << std::endl;
    }
    if (g5(1) >= 1e-4) {
        std::cout << "        TalosSingleStepPeriodicityConstraints.cpp: translation friction cone is violated: " 
                  << g5(1)
                  << std::endl;
    }
    if (g5(2) >= 1e-4) {
        std::cout << "        TalosSingleStepPeriodicityConstraints.cpp: rotation friction cone is violated: " 
                  << g5(2)
                  << std::endl;
    }
    if (g5(3) >= 1e-4) {
        std::cout << "        TalosSingleStepPeriodicityConstraints.cpp: ZMP on one axis is violated: " 
                  << g5(3)
                  << std::endl;
    }
    if (g5(4) >= 1e-4) {
    ...
    if (g5(5) >= 1e-4) {
        std::cout << "        TalosSingleStepPeriodicityConstraints.cpp: ZMP on the other axis is violated: " 
                  << g5(5)
                  << std::endl;
    }
    if (g5(6) >= 1e-4) {
